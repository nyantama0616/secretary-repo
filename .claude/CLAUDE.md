...
- 日報・タスク管理アプリ（Web UI）の構築
- AI エージェント（OpenClaw）向けの MCP サーバーの構築

AI エージェント自体の設定（プロンプト、ツール構成など）は別リポジトリで管理する。
MCP がこのリポジトリと AI エージェントの境界であり、エージェント側はこのアプリの内部構造を知る必要がない。
